...
const InteractiveType = ({
  ...
  const handleFileChange = (event) => {
    ...
    const formatError = validateFileFormat(file, formData.headerType);
    if (formatError) {
    ...
    // Check file size
    const maxSize = getMaxFileSize(formData.headerType);
  ...
  // Validate file format based on header type
  const validateFileFormat = (file, headerType) => {
  ...
  const getFileAcceptType = (headerType) => {
    switch (headerType) {
      case "Image":
        return ".png,.jpg,.jpeg";
      case "Video":
  ...
  const getUploadButtonText = () => {
    ...
    const maxSizeMB = headerType === "Image" ? 5 : 16;
  ...
  const renderFileTooltip = (props) => (
    <Tooltip id="file-info-tooltip" {...props}>
      Allowed types: {(formData.headerType === "Image" ? "PNG, JPG, JPEG" : 
                      formData.headerType === "Video" ? "MP4" : 
                      "PDF, DOC, DOCX, CSV, XLS, XLSX")}
      <br />
      Max size: {formData.headerType === "Image" ? "5MB" : "16MB"}
    </Tooltip>
  );
        ...
        {(formData.headerType === "Image" || formData.headerType === "Video" || formData.headerType === "Document") && (
          <div className="d-flex justify-content-center">
            <input
              ...
              style={{ display: "none" }}
              accept={getFileAcceptType(formData.headerType)}
            />
  ...
  const handleHeaderTypeChange = (type) => {
    handleInputChange("headerType", type);
    setUploadedFile(null); // Reset uploaded file when header type changes
  ...
  return (
    <>
      <div className="form-group">
        <div className="type-heading-wrapper">
          <h6 className="type-heading">Interactive Type</h6>
        </div>
        <div className="new-flex">
          <label>
            <input
              ...
              value="Button"
              checked={formData.interactiveType === "Button"}
              onChange={(e) =>
                handleInputChange("interactiveType", e.target.value)
              }
            />
            Button
          </label>
          <label>
            <input
              ...
              name="interactiveType"
              value="List"
              checked={formData.interactiveType === "List"}
              onChange={(e) =>
                handleInputChange("interactiveType", e.target.value)
              }
            />
      ...
      <div className="form-group">
        <div className="type-heading-wrapper">
          <h6 className="text-center type-heading">Header</h6>
        </div>

        <div className="new-flex">
          {formData.interactiveType === "Button" ? (
            <>
              <label>
                <input
                  className="form-check-input form-round"
              ...
              <label>
                <input
                  ...
                  name="headerType"
                  value="Video"
                  checked={formData.headerType === "Video"}
                  onChange={(e) => handleHeaderTypeChange(e.target.value)}
                />
                Video
              </label>
              <label>
                <input
                  ...
                  name="headerType"
                  value="Document"
                  checked={formData.headerType === "Document"}
                  onChange={(e) => handleHeaderTypeChange(e.target.value)}
                />
                Document
              </label>
        ...
        {formData.headerType && formData.headerType !== "None" && (
          <div className="header-additional-options">
            {renderHeaderOptions()}
          </div>
        )}
      </div>
