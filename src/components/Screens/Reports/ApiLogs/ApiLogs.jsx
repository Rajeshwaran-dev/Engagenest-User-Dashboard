...
const ApiLogs = () => {
  ...
  const { data: chartData, isLoading: chartLoading, error: chartError } = useGetApiBroadCastChartQuery({
  ...
  // Fetch all API campaigns
  const { data: campaigns = [], isLoading: campaignsLoading } = useGetAllApiCampaignsQuery();
  ...
  const { data: campaignDetails, isLoading: detailsLoading, refetch: refetchCampaignDetails } = useGetCampaignDataQuery(
    {
      id: selectedCampaign?._id?.campaignId || selectedCampaign?._id?.erpType,
      offset: (detailCurrentPage - 1) * itemsPerPage,
      limit: itemsPerPage,
      number: searchTerm || null,
      startDate: selectedDateRange.startDate, // Added date range
      endDate: selectedDateRange.endDate       // Added date range
    },
  ...
  const handleDownloadReport = () => {
    const campaignId = selectedCampaign?._id?.campaignId || selectedCampaign?._id?.erpType;
    ...
    fetch(url, {
      ...
      .then(blob => {
        ...
        a.href = url;
        a.download = `${campaignId}-api-report.csv`;
                      ...
                      <table className="table bordered-table mb-0">
                        <thead>
                          <tr>
                            ...
                            <th scope="col" className="text-center">Publish Date</th>
                            {selectedCampaign?._id?.erpType && (
                              <th scope="col" className="text-center">Order Type</th>
                            )}
                            ...
                            <div style={{
                              ...
                              {selectedRecord.data.template?.header && (
                                <div style={{ marginBottom: "8px" }}>
                                  ...
                                  {selectedRecord.data.template?.headerType === 'document' && (
                                    <div style={{ textAlign: "center", padding: "10px" }}>
                                      <Icon icon="ph:file-text" style={{ fontSize: "32px", color: "#667781" }} />
                                    </div>
                                  )}
                                </div>
                              )}
                              ...
                              <p style={{
                                ...
                                {selectedRecord.data.template?.message ||
                                  (selectedCampaign._id?.campaignId || selectedCampaign._id?.erpType)}
                              </p>
